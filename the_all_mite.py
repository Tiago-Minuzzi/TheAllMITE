...
r1 = MODEL_1.predict(sequence_arrays)
results_1 = np.argmax(r1, axis=1)
accuracy_1 = np.max(r1, axis=1)
df1 = pd.DataFrame(test_records)
df1['prediction_1'] = results_1
df1['accuracy_1'] = accuracy_1
...
not_mite_pred_1 = not_mite_pred_1.loc[:, ['id', 'len', 'prediction_1', 'accuracy_1']]

# Concatenate sequences filtered as not MITE by length and the ones classified as not MITE
not_mite = pd.concat([not_mite_filtered, not_mite_pred_1]).fillna(1)
not_mite['prediction_2'] = not_mite['prediction_1']
not_mite['accuracy_2'] = not_mite['accuracy_1']
...
r2 = MODEL_2.predict(sequence_arrays)
results_2 = np.argmax(r2, axis=1)
accuracy_2 = np.max(r2, axis=1)
mites['prediction_2'] = results_2
mites['accuracy_2'] = accuracy_2
mites['prediction_2'] = mites['prediction_2'].map({0: 'Stowaway', 1: 'Tourist'})
mites = mites.loc[:, ['id', 'len', 'prediction_1', 'accuracy_1', 'prediction_2', 'accuracy_2']]
print(">>> Step 03 done!\n")

# Create final table
final_results = pd.concat([mites, not_mite]).round(4)
